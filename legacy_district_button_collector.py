...
class DistrictButtonCollector:
    def __init__(self):
        ...
        # 🚀 API 방식 추가 설정 (정확한 범위로 수정)
        self.seoul_districts_coords = {
            '강남구': {'lat': 37.517, 'lon': 127.047, 'btm': 37.492, 'lft': 127.012, 'top': 37.542, 'rgt': 127.082},
    ...
    def process_api_property(self, prop, district_name):
        ...
        try:
            ...
            # 좌표 정보 추가
            lat = float(prop.get('lat', 0)) if prop.get('lat') else 0
            lng = float(prop.get('lng', 0)) if prop.get('lng') else 0
